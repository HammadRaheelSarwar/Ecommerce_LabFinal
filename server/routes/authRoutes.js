...
router.post('/register', expressAsyncHandler(async (req, res) => {
  ...
  // Auto-verify all users for now to bypass the email verification step
  ...
  const user = await User.create({
    ...
    otpCode: isAutoVerified ? '' : otpHash,
    otpExpiry: isAutoVerified ? undefined : new Date(Date.now() + otpExpiryMinutes * 60 * 1000),
    otpResendCount: 0,
    otpLastSentAt: isAutoVerified ? undefined : new Date(),
  });
  
  if (user) {
    if (isAutoVerified) {
      // Admins are auto-verified via the secret key, skip OTP email
