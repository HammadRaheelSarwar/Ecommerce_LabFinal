...
const AdminLayout = () => {
  ...
  const isActive = (path) => {
    // Check if the current full path (including search) exactly matches the link
    const currentPath = location.pathname + location.search;
    // For the main dashboard '/admin', we only want to match if it's exactly '/admin' without extra stuff
    ...
    <div className="admin-layout">
      ...
      <aside className="admin-sidebar glass">
        ...
        <nav className="admin-nav" style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          ...
          <Link to="/admin/products?gender=Male" style={linkStyle('/admin/products?gender=Male')}><Package size={18} /> Male Products</Link>
          <Link to="/admin/products?gender=Female" style={linkStyle('/admin/products?gender=Female')}><Package size={18} /> Female Products</Link>
